...
<jsp:useBean id="compareGroupHitChart" class="depths.presentationModule.teacher.charts.CompareGroupHitChart" scope="session"/>
...
<%--studentStatisticBeanId.setChartsRef(compareGroupHitChart); --%>
<%
...
<div id="content" class="column">
  ...
  <div align="center">
          ...
          <div align="center">
            ...
            <br>
			  <img src="<%= graphURL %>" width=600 height=400 border=0 usemap="#<%= filename %>" alt="">                </div>
 ...
 <td width="42%" rowspan="4" style="vertical-align:middle"><input type=image src="images/button_refresh.png" style="width:80; height:22" name=refresh /></td>
				...
				while (groupIter.hasNext()) {
					...
					if (optionGroup.equals(group)) { %>
                          <option selected="selected"><%= optionGroup %></option>
                          <%					} else { %>
  ...
  <%			Iterator compareGroupIter = compareGroupList.listIterator();
				while (compareGroupIter.hasNext()) {
			      ...
			      <option selected><%= optionCompGroup %></option>
  <%					} else { %>
			      <option><%= optionCompGroup %></option>
  <%					} %>
  <%				} %>
			      </select>			</td>
		  </tr>
