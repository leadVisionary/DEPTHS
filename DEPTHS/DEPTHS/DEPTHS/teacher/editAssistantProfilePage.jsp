...
<div id="content" class="column">
  ...
  <%if (!hasPrivilegies.equals("Limited")){ %>
       ...
       </strong> <%=teacherBeanId.getNewAssistantDataByType("firstName",assistantId) %> &nbsp;<%=teacherBeanId.getNewAssistantDataByType("lastName",assistantId) %>
        <br />
       ...
       <%for (int i=0;i<assistantPrivNumb;i++){%>
         ...
         <%=teacherBeanId.getAssistantPrivilegieNameById(privilegieId)%>,&nbsp;
         <%} %>
         <br />
	  ...
	  <form name="form1" method="post" action="formsAction.jsp?assistantId=<%=assistantId%>">
	  <table width="82%">
	  <tr>
          ...
          <%for (int i=0;i<numbOfPrivilegies;i++){ %>
            ...
            <p>
             ...
             <%if (!assistantHasThatPriv){ %>
              ...
              <%}else{ %>
               <input type="checkbox" name="privilegie" checked="checked" value="<%=privilegieId%>"><%=privilegieName %>
	    ...
	    <td width="15%" rowspan="4" style="border-top: 1px dotted #A6BDF2;padding-top:4; padding-bottom:4" align="center">
              <div align="left">Selected:
                <select multiple size="8" id=selectedGroups name=selectedGroups style="width:100px;" >
                  ...
                  <OPTION value="<%=assistGrId%>"><%=teacherBeanId.getGroupNameById(assistGrId) %></OPTION>
                  <%} %>
