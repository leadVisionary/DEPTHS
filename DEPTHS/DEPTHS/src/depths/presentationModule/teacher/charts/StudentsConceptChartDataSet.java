...
public class StudentsConceptChartDataSet {
  ...
  //}
    ...
    public StudentsConceptChartDataSet(StudentsStatisticBean studStatBean,String courseId,int studentId) throws ParseException {
     studStatisticBean = studStatBean;
                ...
                double degrees[] =studStatisticBean.getConceptsDegrees(courseId,studentId);
                    //{1.1, 1.2, 3.43, 3.12, 2.76, 5.12, 3.403, 4.21, 5.44, 2.34, 3.42, 1.23, 3.76, 4.56, 2.54};
                double knLev[] =studStatisticBean.getActualKnowledgeLevel(courseId,studentId);
                    //{5.0, 3.4, 4.43, 3.72, 3.26, 3.82, 3.63, 3.91, 4.24, 4.04, 3.92, 3.63, 3.71, 3.96, 3.54};
                //String st="Inicial knowledge="+knLev[0];

                for (int i=0;i<concepts.length;i++){
                    data.add(new StudentsConceptHit(concepts[i],"Concepts",i,degrees[i]));
                ...
                for (int i=0;i<concepts.length;i++){
                    data.add(new StudentsConceptHit(concepts[i],"Progress",i,knLev[i]));

                }
                for (int i=0;i<concepts.length;i++){
                                    data.add(new StudentsConceptHit(concepts[i],"Tracking",i,degrees[i]));
                                }
               for (int i=0;i<concepts.length;i++){
                                    data.add(new StudentsConceptHit(concepts[i],"Service",i,knLev[i]));
                                }
    }
                ...
                while (iter.hasNext()) {
                        ...
                        if (filterSection == null ? true : filterSection.equals(cHit.getSection())) {
                                ...
                                if (position == null) {
                                        ...
                                        StudentsConceptHit previousStudentsConceptHit = (StudentsConceptHit)results.get(position.intValue());
                                        previousStudentsConceptHit.setHitDegree(previousStudentsConceptHit.getHitDegree() + cHit.getHitDegree());
                                }
                        }
                }
                return results;
    ...
    public ArrayList getSections() {
        ...
        public static void main(java.lang.String[] args) {
          ...
          try {
                        ...
                        StudentsConceptChartDataSet cDataSet = new StudentsConceptChartDataSet(studStatisticBeanssess);
                        //SimpleDateFormat sdf = new SimpleDateFormat("dd-MMM-yyyy",Locale.UK);
                        String cTitle=null;
